...
const Tag = ({children,color=C.accent}) => (
  <span style={{display:"inline-block",padding:"2px 10px",borderRadius:20,fontSize:11,
    ...
    background:`${color}20`,border:`1px solid ${color}40`,color}}>{children}</span>
);
...
const BackBtn = ({onClick}) => (
  <button onClick={onClick} style={{marginBottom:20,padding:"8px 18px",borderRadius:20,
          ...
          <div key={s} style={{display:"flex",alignItems:"center",flex:i<STEPS.length-1?1:"none"}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              ...
              <span style={{fontSize:10,fontWeight:600,letterSpacing:"0.5px",textTransform:"uppercase",
                color:active?C.accent:done?C.green:C.muted}}>{s}</span>
            </div>
            {i<STEPS.length-1&&(
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div>
          <label style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"1px",
...
function SkillStep({profile,onNext,onBack}) {
  ...
  const examples=[
    "I know React well and have done 3 projects, SQL is decent, Python is beginner level",
    "Strong in Java, done OOP course, a bit of HTML/CSS, zero experience in deployment",
    "C++ expert, competitive programming, started learning web dev recently",
  ];
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
      ...
      <textarea value={input} onChange={e=>setInput(e.target.value)}
        ...
        rows={5} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,
      ...
      <div style={{marginTop:10,marginBottom:20}}>
        ...
        {examples.map((ex,i)=>(
          <button key={i} onClick={()=>setInput(ex)}
            style={{marginRight:6,marginBottom:6,padding:"4px 10px",borderRadius:20,
              border:`1px solid ${C.border}`,background:"transparent",color:C.muted,
      ...
      <div style={{display:"flex",gap:8,marginBottom:20,marginTop:16}}>
        ...
        {!loading&&(
          <button onClick={fetchJobs}
            style={{marginLeft:"auto",padding:"6px 14px",borderRadius:20,border:`1px solid ${C.border}`,
      ...
      {loading?(
        ...
        <>
            ...
            {filtered.map((job,idx)=>{
              ...
              return (
                <div key={jobId} onClick={()=>toggle(jobId)}
                  style={{background:isSel?C.accentLo:C.card,
          ...
          <div style={{marginTop:20,display:"flex",gap:10}}>
            <div style={{flex:1,background:C.surface,borderRadius:12,padding:"12px 16px",
...
function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"40px 20px",boxSizing:"border-box",
